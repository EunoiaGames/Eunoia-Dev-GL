...
namespace Eunoia
{
	...
	struct v3
	{
		...
		inline v2 yz() const { return v2(y, z); }
		inline v2 xz() const { return v2(x, z); }
	...
	struct v4
	{
		...
		inline v4 operator*(r32 scaler)	   const { return v4(x * scaler, y * scaler, z * scaler, w * scaler); }
		inline v4 operator*(const v4& vec) const { return v4(x * vec.x, y * vec.y, z * vec.z, w * vec.w); }
		inline v4 operator+(const v4& vec) const { return v4(x + vec.x, y + vec.y, z + vec.z, w + vec.w); }
		inline v4 operator-(const v4& vec) const { return v4(x - vec.x, y - vec.y, z - vec.z, w - vec.w); }

		inline v4& operator+=(const v4& vec) { x += vec.x; y += vec.y; z += vec.z; w += vec.w; return *this; }
		inline v4& operator*=(r32 scaler) { x *= scaler; y *= scaler; z *= scaler; w *= scaler; return *this; }
		inline v4& operator*=(const v4& vec) { x *= vec.x; y *= vec.y; z *= vec.z; w *= vec.w; return *this; }
	...
	struct m4
	{
		...
		inline static m4 CreatePerspective(r32 ar, r32 fov, r32 znear, r32 zfar)
		{
			r32 tanHalfFov = tanf(EU_DEG_TO_RAD(fov) / 2.0f);
			r32 zrange = znear - zfar;
	...
	struct Transform2D
	{
		...
		inline Transform2D& Translate(const v2& dir, r32 amt) { pos += (dir * amt); return *this; }
		...
		inline void Rotate(r32 deg) { rot = fmod(rot + deg, 360); }
	...
	struct Transform3D
	{
		...
		inline m4 CreateTransformMatrix() { return m4::CreateTransformation(pos, scale, rot); }
